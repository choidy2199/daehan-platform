...
export async function POST(request: NextRequest) {
  try {
    ...
    const { brand, model, product_name, spec, pallet_qty, base_fob_usd, management_code, internal_code, erp_code, memo } = body || {};
    if (!brand || !model) {
      return NextResponse.json({ success: false, error: '브랜드, 모델은 필수입니다' }, { status: 400 });
    }
    const baseFob = Math.max(0, Number(base_fob_usd) || 0);
    // erp_code (legacy) → management_code 폴백
    const mgmtCode = management_code !== undefined ? management_code : erp_code;
    const { data, error } = await supabase
      .from('import_products_v2')
      .insert({
        ...
        management_code: mgmtCode?.toString().trim() || null,
        internal_code: internal_code?.toString().trim() || null,
        memo: memo?.trim() || null,
      })
      .select('*')
      .single();
...
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json();
    const { id, brand, model, product_name, spec, pallet_qty, base_fob_usd, management_code, internal_code, erp_code, memo } = body || {};
    ...
    // erp_code (legacy) → management_code 폴백
    if (management_code !== undefined) payload.management_code = management_code?.toString().trim() || null;
    else if (erp_code !== undefined) payload.management_code = erp_code?.toString().trim() || null;
    if (internal_code !== undefined) payload.internal_code = internal_code?.toString().trim() || null;
