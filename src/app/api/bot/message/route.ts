...
function formatProductDataForPrompt(products: MatchedProduct[], allRaw: Product[]): string {
  return products.slice(0, 10).map(p => {
    ...
    const name = p.name || raw?.name || raw?.description || '';
    return `모델명: ${p.model} | 품명: ${name} | A단가: ${outA > 0 ? outA.toLocaleString('ko-KR') : '?'}원 | 공급가: ${supplyPrice.toLocaleString('ko-KR')}원 | 자사재고: ${stock}개 | 밀워키재고: ${ttiStock || '?'}`;
  }).join('\n');
}
...
- 자기소개 요청 시: "안녕하세요 대한종합상사 영업지원팀 팀장 Leo Choi입니다. Leo팀장 또는 레오팀장이라고 불러주세요!"
...
여러건 나열 예시:
"M18 임팩 관련 제품 안내드립니다.
🔵 FID3-502X 임팩트드라이버 세트 - 340,000원
🔵 FID3-0X0 임팩트드라이버 베어 - 195,000원
🔵 FHIW-502X 임팩트렌치 세트 - 380,000원
어떤 모델인지 확인부탁드립니다"
...
- 후보가 2~5개면 모델명과 간단한 설명을 나열하고 확인 요청
