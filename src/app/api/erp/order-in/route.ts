...
export async function POST(request: NextRequest) {
  try {
    ...
    const xml = await callNewOrderIn(info, itemsStr, ibgum);

    console.log(`[NewOrderIn] 응답 길이: ${xml.length}`);

    // 응답에서 결과 추출
    const resultMatch = xml.match(/<NewOrderInResult>([^<]*)<\/NewOrderInResult>/) ||
                         xml.match(/<string[^>]*>([^<]*)<\/string>/);
    const result = resultMatch ? resultMatch[1].trim() : xml.substring(0, 200);

    console.log(`[NewOrderIn] 결과: ${result}`);
    ...
    console.error('[NewOrderIn Error]', err);
