...
function DashboardTab() {
  ...
  useEffect(() => {
    ...
    fetch('/api/ad/campaigns')
      ...
      .catch(e => {
        if (cancelled) return;
        setError(e?.message || '캠페인 데이터를 가져오지 못했습니다');
      })
      .finally(() => {
  ...
  const dailyBudgetSum = campaigns
  ...
  campaigns.forEach(c => {
    if (c.status === 'ELIGIBLE' && (c.daily_budget ?? 0) === 0)
      issues.push({ campaign: c, rule: 'A', message: '일예산 0원 → 광고가 노출되지 않습니다' });
    if (c.status === 'PAUSED' && (c.daily_budget ?? 0) > 0)
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      <Section
        title="📋 캠페인 목록"
        sub={isLoading ? '불러오는 중...' : error ? '오류' : `${totalCount}개 · 행 클릭 시 상세 펼침`}
      >
        {error ? (
          <div style={{ background: '#FCEBEB', borderRadius: 8, padding: '16px 20px', color: '#C92B2B', fontSize: 13, lineHeight: 1.7 }}>
            ⚠️ 캠페인 데이터를 가져오지 못했습니다<br />
            <span style={{ fontSize: 12, opacity: 0.85 }}>{error}</span><br />
            <span style={{ fontSize: 12, opacity: 0.7 }}>→ 잠시 후 [↻ 동기화] 버튼을 눌러 다시 시도해주세요</span>
          </div>
        ) : isLoading ? (
                    ...
                    >
                      ...
                      <td
                        style={{
                          ...TD,
                          textAlign: 'right',
                          color: (c.daily_budget ?? 0) === 0 ? C.danger : C.text,
                          fontWeight: (c.daily_budget ?? 0) === 0 ? 600 : 400,
                        }}
                      >
                        {fmtKRW(c.daily_budget ?? 0)}
                      </td>
                      <td style={{ ...TD, color: C.textSec }}>{(c.reg_tm ?? '').slice(5, 10)}</td>
                    </tr>
                    {isOpen && (
                      <tr style={{ background: '#F8FAFD' }}>
                        ...
                        >
                          ...
                          <div>마지막 동기화: {c.synced_at ? absTime(c.synced_at) : '—'}</div>
